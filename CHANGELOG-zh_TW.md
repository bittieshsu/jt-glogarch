...
- **匯入／匯出進度條在工作仍執行時不再凍結。** 前景進度原本只靠即時 SSE 串流更新;2 秒的輪詢備援被 `sseOk` 條件擋住,而 `sseOk` 在第一筆事件與每次心跳都會被設為 true。因此只要 SSE 串流卡住或斷線(長時間匯入經過反向代理時很常見,或 backpressure 暫停期間),最上方進度條就會停在舊數字,即使工作在後端持續前進、健康資訊行也持續更新——看起來就像匯入「只有在背景才會跑」。現在進度條會在每 2 秒的輪詢從工作紀錄本身刷新(共用單調遞增的繪製器,不會凍結也不會倒退)。**釐清:工作一律在後端執行,與對話框無關——「關閉(背景繼續)」只是把畫面收起來,從來就不是啟動開關。**

- **匯入進度對話框的「批次」下拉選單套用主題樣式。** 它是 `no-custom` 原生 `<select>`(由狀態輪詢以程式更新),被自訂下拉的皮膚略過,因而顯示成瀏覽器原生樣式;現在改為與其他有邊框／圓角的控制項一致,並加上自訂箭頭。

### 變更

- 匯入健康資訊標籤改為「本機 jt-glogarch」（本機字樣置前），與上方「目標 Graylog」一致。

## [1.13.43] - 2026-07-24

### 新增

- **儀表板顯示「預估可再存多久」＋ 空間不足告警。** 「可用磁碟空間」卡片現在會顯示例如「≈ 可再存約 5 個月」的估算,依歸檔**自身資料**計算:每月 log 的壓縮量（總壓縮大小 ÷ 歸檔涵蓋的 log 時間跨度）× 可用磁碟空間。不做固定壓縮比／速率假設——它已內含這套環境實際的壓縮比與資料量,並會隨資料成長重新推算。歸檔時間跨度未滿 1 天前會顯示「資料累積中,尚無法估算」。每日內部檢查會在估算低於 `retention.disk_alert_months`（預設 1.0 個月;設 0 關閉）時發送通知。`/api/status` 新增 `retention_estimate` 與 `disk_alert_months` 欄位。

## [1.13.42] - 2026-07-24

### 新增

- **匯入時明確的「暫停中／等待」橫幅。** 當匯入被目標端 backpressure（輸出緩衝滿載／journal 待處理量過高／JVM heap 過高）或本機記憶體不足自動節流到暫停時，原本靜止不動的進度條看起來像「卡住」。現在進度對話框會顯示琥珀色橫幅，並指出主要的壅塞來源——例如「暫停中：正在等待 目標 Graylog 輸出緩衝（97%）消化。會自動繼續——可按『關閉（背景繼續）』」。手動暫停也會明確標示。讓操作者知道靜止的進度條是在**等待**，不是掛掉。

## [1.13.41] - 2026-07-24

### 修正

- **匯入進度在暫停時不再誤報「SSE timeout」。** 當目標 Graylog 的輸出緩衝滿載（backpressure）時，匯入會暫停並停止送出進度事件；原本即時進度串流在 10 分鐘沒有事件後會誤報 `SSE timeout`，把仍在執行的匯入畫成紅色錯誤。現在暫停期間會定期送出心跳（heartbeat），並且只以工作的實際狀態（最後一筆事件，或工作離開記憶體後改讀資料庫）結束串流——暫停或緩慢的匯入不會再被顯示為失敗。瀏覽器端也會保留輪詢備援，不會把仍在執行的工作提早判定完成。

### 變更

- **匯入前容量對話框：資訊性文字改為淡色，不再是橘色。** 磁碟／量測來源／SOP 等資訊行原本使用警告色，即使結論是「足夠」也看起來像警示；現在改為中性淡色，只有真正「不足」的行才會凸顯。中文對話框的冒號也補上全形間距，不再擠在一起。

## [1.13.40] - 2026-07-24

### 修正

- **匯入前的預估與實際匯入現在會一致。** 對話框顯示的是**實測**索引數（例如 74）,但實際匯入時 preflight
  又用原始 JSON 重算容量（例如 165）並中止——「對話框說 74、匯入卻說 165、要調到 165」。現在會把對話框
  的實測索引數帶進匯入,讓 `check_capacity` 用**同一個**操作者看到的數字,不再出現不一致的中止。
- **容量預估讀錯 OpenSearch 磁碟。** 先前量測的是 jt-glogarch **全域設定**的 OpenSearch（常常是另一套／
  正式叢集）,而不是匯入**目標自己**的 OpenSearch——例如顯示 1.97 TB,但目標的 data 路徑其實是 4 TB 的
  磁碟。現在會**優先量測目標的** OpenSearch（明確的 `target_os_url` → 從目標 Graylog 主機自動偵測 →
  最後才用全域 OS）,而且預估會**顯示量測自哪個主機與 data 路徑**,一眼就能看出有沒有讀錯叢集。

## [1.13.39] - 2026-07-24

### 修正

- **容量檢查不再「擋住」匯入——新增「仍要匯入」。** preflight 在保留策略「會刪除資料」時會中止匯入,但
  預估可能高估,導致原本沒問題的匯入變成無法進行。現在容量警告會提供**「仍要匯入」**按鈕（並有一條
  `ignore_capacity` 旗標貫穿前後端）,可在明確確認後略過檢查繼續匯入。
- **容量預估不再被非 log 的系統索引灌大。** 當目標索引集是空的時,會從叢集量測每筆位元組數——但單純的
  叢集平均會被小型系統／外掛索引（`top_queries`、`.kibana`、`gl-events`——筆數少但單筆大）拉高。現在只
  加總真正的 **log** 索引（排除點開頭與已知系統索引）,讓量測到的索引大小更貼近實際（測試叢集上:
  160 B/doc,而非被拉高的 407 → 索引數少約 2.5 倍）。

## [1.13.38] - 2026-07-24

### 修正

- **容量預估對「空的還原目標」會高估索引數。** 索引數本應以**實測**的 OpenSearch 每筆位元組數計算,但先前
  只從目標索引集自己的索引讀取——而全新的還原目標沒有任何索引（「currently has 0 indices」）,於是退回用
  原始（未壓縮）封存 JSON 大小,把索引數灌大（例如 3.4 TB JSON → 162 個索引,但實際落地小很多）。現在會退回
  從**整個叢集既有的資料**量測每筆位元組數,讓預估即使目標索引集是空的,也能反映 OpenSearch 的真實落地
  大小。（只有在整個叢集完全沒有資料時,才會用原始 JSON。）

## [1.13.37] - 2026-07-24

### 新增

- **匯入前容量預估現在會讀取真實的 OpenSearch data 路徑磁碟,並建議保留設定。** 先前只跟索引集的
  `max_number_of_indices` 比較。現在會:
  - 讀取實際的 **OpenSearch data 路徑磁碟**（總量＋可用,透過 `_nodes/stats/fs`）,所以能抓到「磁碟其實
    夠、但保留數設太少」的情況;
  - 從目標既有索引**實測每筆文件的索引大小**,讓索引數預估反映 OpenSearch **實際落地大小**,而非原始
    封存 JSON 大小（兩者不同——會顯示實測的「索引 ≈ 原始 JSON 的 N 倍」比例）;
  - 建議**在磁碟 80% 內可容納的最大索引保留數**（含副本計算）,若連 80% 都裝不下會警告;
  - 提供**一鍵「將保留數設為 N」**（只調高 `max_number_of_indices`,不會調低／刪資料）以及手動 SOP
    （Graylog → System → Indices → 索引集 → Edit → Index Retention）。

## [1.13.36] - 2026-07-24

### 新增

- **匯入前容量預估。** 匯入對話框現在會在你開始前,先顯示目標索引集的循環＋保留設定夠不夠裝下這批
  資料:「本次匯入:N 筆／X GB → 約需 M 個索引 · 足夠／可能不足——建議先調高 max_number_of_indices」。
  依所選歸檔自動計算。
- **匯入後的目標端驗證。** 完成時會顯示目標**實際入庫**了多少筆（送出數扣掉 indexer failures）,例如
  「已完成（5,235,180 筆）· 目標端入庫 5,235,053 筆」——讓你確認「真正到目的地」的筆數,而不只是送出數。
- **歸檔份數旁並陳筆數。** 「匯入工作已啟動：…（50 份歸檔）」現在會一併顯示總筆數（例如「50 份歸檔 ·
  5,235,180 筆」）,避免把份數誤會成筆數。

### 修正

- **防止「全選＝只有這一頁」造成的不完整匯入。** 當你批次匯入的選取數少於「符合目前篩選」的總數時,現在
  會跳出對話框強制明確選擇——**匯入全部 {total} 份** 或 **只匯入這 {selected} 份**——讓「只匯入多頁範圍中
  的一頁」（例如一個月只進了一天）不可能再無聲地發生。

## [1.13.35] - 2026-07-24

### 新增

- **indexer failures 現在會在匯入「過程中」就修正,而不是只在匯入完成後。**
  - **Bulk 模式——真正零遺失。** `_bulk` 回應會逐筆回報錯誤,所以某批遇到對應衝突時,jt-glogarch 會把
    出問題的欄位釘選為 string、循環索引,並在**同一次匯入內把失敗的文件重送一次**——不會遺失任何一筆。
  - **GELF 模式——提早止血。** GELF 是送出即忘（收不到逐筆回報）,所以匯入現在會在既有的背壓檢查節奏
    上輪詢目標的 indexer-failure 數量;一偵測到上升就把欄位釘選為 string 並循環索引,讓**後續**的匯入
    正常建立索引,而不是每一筆衝突訊息都失敗到最後。（偵測前送出的少數幾筆,用下面的重試補回。）
- **失敗匯入的「一鍵重試」。** 遇到 indexer failures 的匯入作業,現在會在作業歷程顯示「重試」按鈕。它會
  重新匯入那次作業的歸檔,優先用 **Bulk**（依訊息 id 去重 → 只補上先前失敗的訊息,不會重複）;若是
  Data Node（無法用 Bulk）則退回 GELF 並顯示明確的重複警語。作業會保存自己的歸檔清單與目標（不含密鑰）
  以支援此功能。
...
- **匯入對話框的「批次間隔（ms）」滑桿被擠成一小條。** 一條通用的 `.form-group input { width: 100% }`
  規則的優先度高過 `.rate-num` 的固定寬度,導致數字輸入框被撐成滿版、旁邊的滑桿縮成約 26px。現在把
  數字框固定回原本的寬度,滑桿就會重新填滿整列。
